...
export function FinanceiroPage() {
  ...
  const loadData = async () => { setLoading(true); try { const [r, s] = await Promise.all([financeiroService.list(), financeiroService.getStats()]); setData(r.data || []); setStats(s); } catch { toast.error('Erro ao carregar'); } finally { setLoading(false); } };
  useEffect(() => { loadData(); }, []);
  ...
  const handleSave = async () => { try { await financeiroService.create(form as any); toast.success(form.category === 'receita' ? 'Receita registrada! 💰' : 'Despesa registrada! 📋'); setModalOpen(false); setForm({ description: '', amount: 0, category: 'despesa', subcategory: '', dueDate: format(new Date(), 'yyyy-MM-dd'), accountId: '', status: 'pendente' }); loadData(); } catch { toast.error('Erro ao salvar'); } };

  const handlePay = async (id: string) => { try { await financeiroService.update(id, { status: 'pago', paidAt: new Date().toISOString() } as any); toast.success('Marcado como pago!'); loadData(); } catch { toast.error('Erro'); } };
  ...
  return (
    <motion.div initial={{ opacity: 0, y: 8 }} animate={{ opacity: 1, y: 0 }} className="space-y-5">
      ...
      <div className="flex flex-col sm:flex-row sm:items-end justify-between gap-4">
        <div>
          <div className="flex items-center gap-2 mb-1"><span className="text-xs text-gray-400 font-medium">Financeiro</span><ChevronRight size={12} className="text-gray-400" /><span className="text-xs text-gray-600 dark:text-gray-300 font-semibold">Transações</span></div>
          <h1 className="text-3xl font-bold text-gray-900 dark:text-white tracking-tight">Financeiro</h1>
          <div className="flex items-center gap-3 mt-2">
            <span className="text-sm text-gray-500">{data.length} transações</span><span className="w-1 h-1 rounded-full bg-gray-300" />
            <span className="text-sm text-emerald-600 font-semibold">Saldo: R$ {(stats.balance / 100).toFixed(2)}</span><span className="w-1 h-1 rounded-full bg-gray-300" />
            <span className="text-sm text-amber-600">{filteredStats.pendingCount} pendentes</span>
          </div>
        </div>
        <div className="flex items-center gap-2">
          ...
          <button onClick={() => setModalOpen(true)} className="flex items-center gap-2 px-5 py-2.5 bg-gradient-to-r from-green-500 to-emerald-600 text-white rounded-xl text-sm font-semibold hover:from-green-600 hover:to-emerald-700 shadow-lg shadow-green-500/25"><Plus size={16} /> Nova Transação</button>
        </div>
      </div>
      ...
      <div className="grid grid-cols-2 sm:grid-cols-4 lg:grid-cols-7 gap-2">
        {[
          { label: 'Saldo Total', value: `R$ ${(stats.balance / 100).toFixed(2)}`, icon: Wallet, color: 'from-emerald-500 to-teal-500' },
          { label: 'Receitas', value: `R$ ${(stats.revenue / 100).toFixed(2)}`, icon: ArrowUpCircle, color: 'from-blue-500 to-indigo-500' },
          { label: 'Despesas', value: `R$ ${(stats.expenses / 100).toFixed(2)}`, icon: ArrowDownCircle, color: 'from-red-500 to-rose-500' },
          { label: 'Pendentes', value: filteredStats.pendingCount, icon: Clock, color: 'from-amber-500 to-orange-500', action: () => setFilterCat('todas') },
          { label: 'Pagos', value: filteredStats.paidCount, icon: CheckCircle, color: 'from-emerald-500 to-teal-500' },
          { label: 'Receitas (período)', value: `R$ ${(filteredStats.revenue / 100).toFixed(2)}`, icon: TrendingUp, color: 'from-violet-500 to-purple-500', action: () => setFilterCat('receita') },
          { label: 'Despesas (período)', value: `R$ ${(filteredStats.expenses / 100).toFixed(2)}`, icon: TrendingDown, color: 'from-pink-500 to-rose-500', action: () => setFilterCat('despesa') },
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-4">
        <div className="glass-card p-5 lg:col-span-2">
          <div className="flex items-center justify-between mb-3">
            <h3 className="text-sm font-semibold text-gray-900 dark:text-white">Fluxo de Caixa</h3>
            <div className="flex gap-1 p-0.5 rounded-lg bg-black/[0.02] dark:bg-white/[0.02]">
              {[{ label: '30 dias', value: 'fluxo' as const }, { label: 'Mensal', value: 'categorias' as const }].map(t => (
                <button key={t.value} onClick={() => setChartTab(t.value)} className={`px-3 py-1 rounded-md text-[10px] font-medium transition-all ${chartTab === t.value ? 'bg-white dark:bg-white/10 shadow-sm text-gray-900 dark:text-white' : 'text-gray-500'}`}>{t.label}</button>
              ))}
            </div>
          </div>
          <div className="h-56"><ResponsiveContainer width="100%" height="100%">
            {chartTab === 'fluxo' ? <AreaChart data={chartData}><defs><linearGradient id="rev" x1="0" y1="0" x2="0" y2="1"><stop offset="5%" stopColor="#10b981" stopOpacity={0.3} /><stop offset="95%" stopColor="#10b981" stopOpacity={0} /></linearGradient><linearGradient id="exp" x1="0" y1="0" x2="0" y2="1"><stop offset="5%" stopColor="#f43f5e" stopOpacity={0.2} /><stop offset="95%" stopColor="#f43f5e" stopOpacity={0} /></linearGradient></defs><CartesianGrid strokeDasharray="3 3" stroke="#e5e7eb" strokeOpacity={0.3} /><XAxis dataKey="date" tick={{ fontSize: 10 }} stroke="#9ca3af" /><YAxis tick={{ fontSize: 10 }} stroke="#9ca3af" /><Tooltip contentStyle={{ borderRadius: 12, border: '1px solid rgba(0,0,0,0.08)', background: 'rgba(255,255,255,0.95)', backdropFilter: 'blur(10px)' }} formatter={(v: number) => `R$ ${v.toFixed(2)}`} /><Area type="monotone" dataKey="receita" stroke="#10b981" fill="url(#rev)" strokeWidth={2} name="Receitas" /><Area type="monotone" dataKey="despesa" stroke="#f43f5e" fill="url(#exp)" strokeWidth={2} name="Despesas" /></AreaChart>
            : <BarChart data={monthlyBalance}><CartesianGrid strokeDasharray="3 3" stroke="#e5e7eb" strokeOpacity={0.3} /><XAxis dataKey="month" tick={{ fontSize: 10 }} stroke="#9ca3af" /><YAxis tick={{ fontSize: 10 }} stroke="#9ca3af" /><Tooltip contentStyle={{ borderRadius: 12, border: '1px solid rgba(0,0,0,0.08)', background: 'rgba(255,255,255,0.95)', backdropFilter: 'blur(10px)' }} formatter={(v: number) => `R$ ${v.toFixed(2)}`} /><Bar dataKey="receita" fill="#10b981" radius={[4, 4, 0, 0]} name="Receitas" /><Bar dataKey="despesa" fill="#f43f5e" radius={[4, 4, 0, 0]} name="Despesas" /></BarChart>}
      ...
      <AnimatePresence mode="wait">
        {viewMode === 'cards' && (<motion.div key="cards" initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }} className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4">
          <AnimatePresence>{filtered.slice(0, 30).map((item, i) => (<motion.div key={item.id} layout initial={{ opacity: 0, scale: 0.95 }} animate={{ opacity: 1, scale: 1 }} exit={{ opacity: 0, scale: 0.9 }} transition={{ delay: i * 0.02 }}
            ...
            {item.status === 'pendente' && (<button onClick={e => { e.stopPropagation(); handlePay(item.id); }} className="mt-2 w-full text-center py-1.5 rounded-lg bg-emerald-500/10 text-emerald-600 text-xs font-semibold hover:bg-emerald-500/20 transition-colors">Marcar como Pago</button>)}
          </motion.div>))}</AnimatePresence>
          {filtered.length === 0 && <div className="col-span-3 flex flex-col items-center py-20 text-gray-400"><DollarSign size={36} className="mb-3 opacity-30" /><p className="text-sm">Nenhuma transação</p><button onClick={() => setModalOpen(true)} className="mt-4 btn-primary text-xs">Nova Transação</button></div>}
        </motion.div>)}

        {viewMode === 'table' && (<motion.div key="table" initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }} className="glass-card overflow-hidden"><div className="overflow-x-auto"><table className="w-full"><thead><tr className="border-b border-white/10 bg-black/[0.01]"><th className="px-4 py-3 text-left text-xs font-semibold text-gray-500 uppercase tracking-wider">Descrição</th><th className="px-4 py-3 text-left text-xs font-semibold text-gray-500 uppercase tracking-wider">Tipo</th><th className="px-4 py-3 text-right text-xs font-semibold text-gray-500 uppercase tracking-wider">Valor</th><th className="px-4 py-3 text-left text-xs font-semibold text-gray-500 uppercase tracking-wider hidden md:table-cell">Vencimento</th><th className="px-4 py-3 text-center text-xs font-semibold text-gray-500 uppercase tracking-wider">Status</th></tr></thead><tbody>{filtered.map(item => (<tr key={item.id} onClick={() => { setSelectedTx(item); setDetailOpen(true); }} className="border-b border-white/5 hover:bg-black/[0.02] cursor-pointer">
      ...
      {/* Create Modal */}
      <Modal open={modalOpen} onClose={() => setModalOpen(false)} title="Nova Transação"><div className="space-y-4">
        <div><label className="text-xs font-semibold text-gray-500 uppercase mb-1.5 block">Descrição *</label><input value={form.description} onChange={e => setForm({ ...form, description: e.target.value })} className="glass-input" placeholder="Ex: Pagamento de aluguel" /></div>
        <div className="grid grid-cols-2 gap-2"><div><label className="text-xs font-semibold text-gray-500 uppercase mb-1.5 block">Valor (centavos) *</label><input type="number" value={form.amount} onChange={e => setForm({ ...form, amount: +e.target.value })} className="glass-input" /></div>
          <div><label className="text-xs font-semibold text-gray-500 uppercase mb-1.5 block">Tipo *</label><select value={form.category} onChange={e => setForm({ ...form, category: e.target.value })} className="glass-input"><option value="receita">📈 Receita</option><option value="despesa">📉 Despesa</option></select></div></div>
        <div className="grid grid-cols-2 gap-2"><div><label className="text-xs font-semibold text-gray-500 uppercase mb-1.5 block">Vencimento</label><input type="date" value={form.dueDate} onChange={e => setForm({ ...form, dueDate: e.target.value })} className="glass-input" /></div>
          <div><label className="text-xs font-semibold text-gray-500 uppercase mb-1.5 block">Categoria</label><input value={form.subcategory} onChange={e => setForm({ ...form, subcategory: e.target.value })} className="glass-input" placeholder="Ex: aluguel, vendas..." /></div></div>
        <button onClick={handleSave} className="btn-primary w-full justify-center text-sm py-3">Registrar Transação</button></div>
      </Modal>

      {/* Detail Modal */}
      <Modal open={detailOpen} onClose={() => setDetailOpen(false)} title={selectedTx?.description || 'Detalhe'}><div className="space-y-4">
        {selectedTx && (<>
          <div className="grid grid-cols-2 gap-3"><div className="glass-card p-3"><p className="text-[11px] text-gray-500">Valor</p><p className={`text-lg font-bold ${selectedTx.category === 'receita' ? 'text-emerald-600' : 'text-red-600'}`}>R$ {(selectedTx.amount / 100).toFixed(2)}</p></div>
            <div className="glass-card p-3"><p className="text-[11px] text-gray-500">Vencimento</p><p className="text-sm font-semibold">{new Date(selectedTx.dueDate || selectedTx.createdAt).toLocaleDateString('pt-BR')}</p></div>
            <div className="glass-card p-3"><p className="text-[11px] text-gray-500">Tipo</p><span className={`text-xs px-2 py-0.5 rounded-full ${selectedTx.category === 'receita' ? 'bg-emerald-50 text-emerald-700' : 'bg-red-50 text-red-700'}`}>{selectedTx.category}</span></div>
            <div className="glass-card p-3"><p className="text-[11px] text-gray-500">Status</p><span className={`text-xs px-2 py-0.5 rounded-full ${getStatusBadge(selectedTx.status)}`}>{selectedTx.status}</span></div></div>
          {selectedTx.status === 'pendente' && <button onClick={() => { handlePay(selectedTx.id); setDetailOpen(false); }} className="btn-primary w-full justify-center text-sm py-3 bg-emerald-500 hover:bg-emerald-600">Marcar como Pago ✓</button>}
        </>)}
      </div></Modal>
    </motion.div>
  );
