...
export function ClientesPage() {
  ...
  const handleSave = async () => {
    try {
      ...
      if (editing) { await clientesService.update(editing.id, payload); toast.success('Cliente atualizado!'); }
      else { await clientesService.create(payload); toast.success('Cliente criado! ✨'); }
  ...
  const handleDelete = async (id: string) => { if (!confirm('Remover permanentemente este cliente?')) return; try { await clientesService.delete(id); toast.success('Cliente removido'); loadData(); } catch { toast.error('Erro ao remover'); } };
  ...
  const handleBulkDelete = async () => { if (!confirm(`Remover ${selectedIds.length} clientes?`)) return; try { await Promise.all(selectedIds.map(id => clientesService.delete(id))); toast.success(`${selectedIds.length} clientes removidos`); setSelectedIds([]); loadData(); } catch { toast.error('Erro ao remover'); } };
  ...
  return (
    <motion.div initial={{ opacity: 0, y: 8 }} animate={{ opacity: 1, y: 0 }} className="space-y-5">
      ...
      <div className="flex flex-col sm:flex-row sm:items-end justify-between gap-4">
        <div>
          <div className="flex items-center gap-2 mb-1">
            ...
            <span className="text-xs text-gray-600 dark:text-gray-300 font-semibold">Clientes</span>
          </div>
          <h1 className="text-3xl font-bold text-gray-900 dark:text-white tracking-tight">Clientes</h1>
          <div className="flex items-center gap-3 mt-2">
            <span className="text-sm text-gray-500">{stats.total} total</span>
            <span className="w-1 h-1 rounded-full bg-gray-300" />
            <span className="text-sm text-emerald-600 dark:text-emerald-400 font-semibold">{stats.ativos} ativos</span>
            <span className="w-1 h-1 rounded-full bg-gray-300" />
            <span className="text-sm text-gray-400">{stats.pf} PF • {stats.pj} PJ</span>
          </div>
        </div>
        <div className="flex items-center gap-2">
          {selectedIds.length > 0 && (
            <motion.button initial={{ scale: 0 }} animate={{ scale: 1 }} onClick={handleBulkDelete}
              ...
              <Trash2 size={15} /> Remover ({selectedIds.length})
            </motion.button>
          )}
      ...
      <div className="grid grid-cols-2 sm:grid-cols-4 gap-3">
        {[
          { label: 'Ativos', value: stats.ativos, icon: CheckCircle, color: 'bg-emerald-500' },
          { label: 'Inativos', value: stats.inativos, icon: AlertCircle, color: 'bg-gray-400' },
          { label: 'Pessoa Física', value: stats.pf, icon: UserPlus, color: 'bg-blue-500' },
          { label: 'Pessoa Jurídica', value: stats.pj, icon: Building2, color: 'bg-violet-500' },
        ].map((s, i) => (
          <motion.div key={s.label} initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: i * 0.05 }}
            className="glass-card p-4 flex items-center gap-3 cursor-pointer" onClick={() => {
              if (s.label === 'Ativos') setFilterStatus('ativo');
              else if (s.label === 'Inativos') setFilterStatus('inativo');
              else if (s.label === 'Pessoa Física') setFilterType('fisica');
      ...
      <div className="glass-card p-3">
        <div className="flex flex-wrap items-center gap-3">
          <div className="relative flex-1 min-w-[220px]">
            ...
            <input value={search} onChange={e => setSearch(e.target.value)} placeholder="Buscar por nome, documento..."
          ...
          <div className="flex items-center gap-1 p-0.5 rounded-xl bg-black/[0.02] dark:bg-white/[0.02]">
            {[{ label: 'Todos', value: 'todos' as FilterStatus }, { label: 'Ativos', value: 'ativo' }, { label: 'Inativos', value: 'inativo' }].map(f => (
          ...
          <select value={filterType} onChange={e => setFilterType(e.target.value as FilterType)}
            ...
            <option value="todos">Todos os tipos</option><option value="fisica">Pessoa Física</option><option value="juridica">Pessoa Jurídica</option>
          </select>
          <button onClick={() => { loadData(); }} className="p-2 rounded-xl hover:bg-black/5 dark:hover:bg-white/5 text-gray-400 transition-colors" title="Atualizar">
      ...
      <AnimatePresence mode="wait">
          ...
          <motion.div key="cards" initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }}
                    ...
                    <div className="flex items-center gap-3">
                      ...
                      <div>
                        ...
                        <div className="flex items-center gap-1.5 mt-0.5">
                          <span className="text-[11px] text-gray-400">{item.type === 'fisica' ? 'PF' : 'PJ'}</span>
            ...
            {filtered.length === 0 && !loading && (
              <div className="col-span-3 flex flex-col items-center justify-center py-20 text-gray-400">
                ...
                <p className="text-xs">Tente ajustar os filtros ou cadastre um novo cliente.</p>
                <button onClick={() => { setEditing(null); setModalOpen(true); }} className="mt-4 btn-primary text-xs">Criar Primeiro Cliente</button>
              </div>
            )}
          </motion.div>
        )}
        ...
        {viewMode === 'table' && (
          <motion.div key="table" initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }}
            ...
            <div className="overflow-x-auto">
              <table className="w-full">
                <thead><tr className="border-b border-white/10 bg-black/[0.01] dark:bg-white/[0.01]">
                  ...
                  <th className="px-4 py-3 text-left text-xs font-semibold text-gray-500 uppercase tracking-wider hidden lg:table-cell">Documento</th>
                  <th className="px-4 py-3 text-left text-xs font-semibold text-gray-500 uppercase tracking-wider hidden xl:table-cell">Cadastro</th>
                ...
                <tbody>
                  {filtered.map(item => (
                    <tr key={item.id} onClick={() => openDetail(item)}
                      ...
                      <td className="px-4 py-3"><div className="flex items-center gap-3"><div className={`w-9 h-9 rounded-xl bg-gradient-to-br ${item.type === 'fisica' ? 'from-blue-400 to-indigo-500' : 'from-violet-400 to-purple-500'} flex items-center justify-center text-white text-xs font-bold`}>{item.name?.charAt(0)}</div><div><p className="text-sm font-medium text-gray-900 dark:text-white">{item.name}</p><p className="text-[11px] text-gray-400">{item.type === 'fisica' ? 'PF' : 'PJ'}</p></div></div></td>
            ...
            {filtered.length === 0 && !loading && <div className="text-center py-16 text-gray-400"><Users size={36} className="mx-auto mb-3 opacity-30" /><p className="text-sm font-medium">Nenhum cliente</p></div>}
          </motion.div>
        )}
      </AnimatePresence>
      ...
      <Modal open={modalOpen} onClose={() => setModalOpen(false)} title={editing ? 'Editar Cliente' : 'Novo Cliente'} size="lg">
        <div className="space-y-4">
          <div className="grid grid-cols-1 sm:grid-cols-3 gap-3">
            <div className="sm:col-span-2"><label className="text-xs font-semibold text-gray-500 uppercase tracking-wider mb-1.5 block">Nome *</label><input value={form.name} onChange={e => setForm({ ...form, name: e.target.value })} className="glass-input" placeholder="Nome completo ou razão social" /></div>
            <div><label className="text-xs font-semibold text-gray-500 uppercase tracking-wider mb-1.5 block">Tipo</label><select value={form.type} onChange={e => setForm({ ...form, type: e.target.value as 'fisica' | 'juridica' })} className="glass-input"><option value="fisica">Pessoa Física</option><option value="juridica">Pessoa Jurídica</option></select></div>
          </div>
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
            ...
            <div><label className="text-xs font-semibold text-gray-500 uppercase tracking-wider mb-1.5 block">Email</label><input value={form.email} onChange={e => setForm({ ...form, email: e.target.value })} className="glass-input" placeholder="email@exemplo.com" /></div>
          </div>
          <div><label className="text-xs font-semibold text-gray-500 uppercase tracking-wider mb-1.5 block">Telefone</label><input value={form.phone} onChange={e => setForm({ ...form, phone: e.target.value })} className="glass-input" placeholder="(11) 99999-9999" /></div>
          <div><label className="text-xs font-semibold text-gray-500 uppercase tracking-wider mb-1.5 block">Observações</label><textarea value={form.notes} onChange={e => setForm({ ...form, notes: e.target.value })} rows={2} className="glass-input" placeholder="Notas internas sobre o cliente..." /></div>
          <div className="pt-2 border-t border-white/10"><label className="text-xs font-semibold text-gray-500 uppercase tracking-wider mb-3 block">Endereço (opcional)</label>
            <div className="grid grid-cols-2 sm:grid-cols-4 gap-2">
              <div><input value={form.cep} onChange={e => setForm({ ...form, cep: e.target.value })} className="glass-input" placeholder="CEP" /></div>
              <div className="sm:col-span-3"><input value={form.street} onChange={e => setForm({ ...form, street: e.target.value })} className="glass-input" placeholder="Rua" /></div>
              <div><input value={form.number} onChange={e => setForm({ ...form, number: e.target.value })} className="glass-input" placeholder="Número" /></div>
              <div><input value={form.complement} onChange={e => setForm({ ...form, complement: e.target.value })} className="glass-input" placeholder="Complemento" /></div>
              <div><input value={form.neighborhood} onChange={e => setForm({ ...form, neighborhood: e.target.value })} className="glass-input" placeholder="Bairro" /></div>
              <div><input value={form.city} onChange={e => setForm({ ...form, city: e.target.value })} className="glass-input" placeholder="Cidade" /></div>
            </div>
          </div>
          <div className="flex items-center gap-4 pt-2">
            <label className="flex items-center gap-2 text-xs text-gray-600 dark:text-gray-400"><input type="checkbox" checked={form.isCustomer} onChange={e => setForm({ ...form, isCustomer: e.target.checked })} className="rounded" /> É cliente</label>
            <label className="flex items-center gap-2 text-xs text-gray-600 dark:text-gray-400"><input type="checkbox" checked={form.isSupplier} onChange={e => setForm({ ...form, isSupplier: e.target.checked })} className="rounded" /> É fornecedor</label>
          </div>
          <button onClick={handleSave} className="btn-primary w-full justify-center text-sm py-3 mt-2">{editing ? 'Salvar Alterações' : 'Cadastrar Cliente'}</button>
        </div>
      </Modal>

      {/* Detail Modal - CRM View */}
          ...
          <div className="space-y-5">
            ...
            <div className="flex items-start gap-5">
              ...
              <div className="flex-1">
                ...
                <p className="text-sm text-gray-500">{selectedClient.type === 'fisica' ? 'Pessoa Física' : 'Pessoa Jurídica'} • Cadastrado em {new Date(selectedClient.createdAt).toLocaleDateString('pt-BR')}</p>
            ...
            <div className="grid grid-cols-2 sm:grid-cols-4 gap-3">
              {[
                { label: 'Compras', value: clientStats.totalSales, icon: ShoppingCart },
                { label: 'Total Gasto', value: `R$ ${(clientStats.totalSpent / 100).toFixed(2)}`, icon: DollarSign },
                { label: 'Ticket Médio', value: `R$ ${(clientStats.avgTicket / 100).toFixed(2)}`, icon: TrendingUp },
                { label: 'Última Compra', value: clientStats.lastPurchase ? new Date(clientStats.lastPurchase).toLocaleDateString('pt-BR') : 'Nunca', icon: Clock },
              ].map((s, i) => (
            ...
            {selectedClient.contacts?.length > 0 && (
              <div className="glass-card p-4">
                <h4 className="text-xs font-semibold text-gray-500 uppercase tracking-wider mb-3">Contatos</h4>
            ...
            {selectedClient.addresses?.[0] && (
              <div className="glass-card p-4">
                <h4 className="text-xs font-semibold text-gray-500 uppercase tracking-wider mb-2">Endereço</h4>
            ...
            {clientSales.length > 0 && (
              <div>
                <h4 className="text-xs font-semibold text-gray-500 uppercase tracking-wider mb-3">Últimas Compras</h4>
                <div className="space-y-1.5">
